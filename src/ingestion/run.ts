...
export async function runIngestion() {
  ...
  return {
    sources: ["CPPP ePublishing", "GeM", "Maharashtra eProcurement", "Madhya Pradesh eProcurement", "West Bengal eTender", "Kerala eTender", "Uttarakhand eTender"],
...
if (import.meta.url === `file://${process.argv[1]}`) {
  runIngestion().then((result) => console.log(JSON.stringify(result, null, 2))).catch((error) => {
    console.error(error);
    process.exitCode = 1;
  });
}
