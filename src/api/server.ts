...
async function listTenders(): Promise<Tender[]> {
  ...
  return (data ?? []).map((row: any) => TenderSchema.parse({
    ...
    publishedAt: row.published_at ?? undefined, closingAt: row.closing_at ?? undefined,
    sourceUrl: row.source_url, documentUrl: row.document_url ?? undefined,
  }));
}
  ...
  const server = createServer(async (req, res) => {
    try {
      ...
      if (req.method === "POST" && url.pathname === "/api/fetch") {
        ...
        return json(res, 200, { source: "configured-json", fetched: tenders.length, upserted, database: hasDatabase() });
      }
      if (req.method === "POST" && url.pathname === "/api/fetch/cppp") {
        const tenders = await fetchCpppTenders();
        const upserted = await upsertTenders(tenders);
        return json(res, 200, { source: "CPPP ePublishing", fetched: tenders.length, upserted, database: hasDatabase() });
      }
      if (req.method === "GET" && url.pathname === "/api/matches") {
        ...
        const tenders = await listTenders();
        return json(res, 200, { matches: tenders.map((t) => ({ tender: t, match: matchTender(t, profile), eligibility: analyzeEligibility(t, profile) })).sort((a, b) => b.match.score - a.match.score) });
      }
