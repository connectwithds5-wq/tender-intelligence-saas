...
import { createBusinessProfile, deleteBusinessProfile, getBusinessProfile, listBusinessProfiles } from "../db/profiles.js";
...
if (req.method === "DELETE" && url.pathname === "/api/profiles") { const profileId = url.searchParams.get("id"); if (!profileId) return json(res, 400, { error: "id is required" }); const user = hasDatabase() ? await currentUser(req) : null; if (hasDatabase() && !user) return json(res, 401, { error: "Login required" }); if (user) await deleteBusinessProfile(profileId, user.id); else { const i = memory.profiles.findIndex((p) => p.id === profileId); if (i >= 0) memory.profiles.splice(i, 1); } return json(res, 200, { ok: true }); }
if (req.method === "POST" && (url.pathname === "/api/fetch" || url.pathname === "/api/fetch/cppp")) { if (!requireIngestionKey(req)) return json(res, 401, { error: "Valid X-Ingestion-Key is required" }); const tenders = url.pathname.endsWith("/cppp") ? await fetchCpppTenders() : await fetchConfiguredFeed(); const upserted = await upsertTenders(tenders); return json(res, 200, { source: url.pathname.endsWith("/cppp") ? "CPPP ePublishing" : "configured-json", fetched: tenders.length, upserted, database: hasDatabase() }); }
if (req.method === "GET" && url.pathname === "/api/matches") { const profileId = url.searchParams.get("profileId"); if (!profileId) return json(res, 400, { error: "profileId is required" }); const user = hasDatabase() ? await currentUser(req) : null; if (hasDatabase() && !user) return json(res, 401, { error: "Login required" }); const profile = user ? await getBusinessProfile(profileId, user.id) : memory.profiles.find((p) => p.id === profileId) ?? null; if (!profile) return json(res, 404, { error: "Profile not found" }); const matches = (await listTenders()).map((tender) => ({ tender, match: matchTender(tender, profile), eligibility: analyzeEligibility(tender, profile) })).sort((a, b) => b.match.score - a.match.score); return json(res, 200, { matches }); }
...
return json(res, 404, { error: "Not found" }); } catch (error) { console.error(error); return json(res, 500, { error: error instanceof Error ? error.message : "Internal server error" }); } }); server.listen(port, () => console.log(`Tender Intelligence SaaS running on http://localhost:${port}`)); return server; }
