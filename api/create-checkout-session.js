...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!process.env.STRIPE_PRICE_ID) return res.status(503).json({ error: "Billing is not configured yet. Add STRIPE_PRICE_ID in Vercel." });

    const client = db();
    const { data: existing } = await client.from("subscriptions").select("stripe_customer_id,plan,status").eq("user_id", user.id).maybeSingle();
    let customerId = existing?.stripe_customer_id;
    if (!customerId) {
      const customer = await stripe("customers", { email: user.email || "", "metadata[user_id]": user.id });
      customerId = customer.id;
      await client.from("subscriptions").upsert({ user_id: user.id, stripe_customer_id: customerId }, { onConflict: "user_id" });
    }
